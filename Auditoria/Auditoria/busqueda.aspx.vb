...
Public Class busqueda
    ...
    Protected Sub getDataSetFromDescripcion()
        ...
        Application("ultimoQueryBusqueda") = "CREATE TABLE [dbo].[#TEMP_REFERENCIAS]([FILA] [int] IDENTITY(1,1) NOT NULL,[NRO_REFERENCIA] [varchar](20) NULL,[DESCRIPCION] [varchar] (80) NOT NULL,[CATEGORIA] [varchar](1) NOT NULL) ON [PRIMARY] INSERT INTO #TEMP_REFERENCIAS (NRO_REFERENCIA,DESCRIPCION,CATEGORIA)SELECT AUD_REFERENCIAS.NRO_REFERENCIA,AUD_REFERENCIAS.DESCRIPCION, AUD_CATEGORIAS.CODIGO FROM AUD_REFERENCIAS INNER JOIN AUD_CATEGORIAS ON AUD_REFERENCIAS.ID_CATEGORIA=AUD_CATEGORIAS.ID WHERE AUD_REFERENCIAS.DESCRIPCION LIKE'" & txtBusqueda.Text & "%' ORDER BY AUD_REFERENCIAS.NRO_REFERENCIA ASC SELECT CATEGORIA,NRO_REFERENCIA,DESCRIPCION FROM #TEMP_REFERENCIAS WHERE FILA>" & (Application("paginaActualBusqueda") - 1) * 5 & " AND FILA<=" & Application("paginaActualBusqueda") * 5 & " ORDER BY LEN(NRO_REFERENCIA),NRO_REFERENCIA ASC"
    ...
    Protected Sub getDataSetFromNroReferencia()
        Application("ultimoQueryBusqueda") = "CREATE TABLE [dbo].[#TEMP_REFERENCIAS]([FILA] [int] IDENTITY(1,1) NOT NULL,[NRO_REFERENCIA] [varchar](20) NULL,[DESCRIPCION] [varchar] (80) NOT NULL,[CATEGORIA] [varchar](1) NOT NULL) ON [PRIMARY] INSERT INTO #TEMP_REFERENCIAS (NRO_REFERENCIA,DESCRIPCION,CATEGORIA)SELECT AUD_REFERENCIAS.NRO_REFERENCIA,AUD_REFERENCIAS.DESCRIPCION, AUD_CATEGORIAS.CODIGO FROM AUD_REFERENCIAS INNER JOIN AUD_CATEGORIAS ON AUD_REFERENCIAS.ID_CATEGORIA=AUD_CATEGORIAS.ID WHERE AUD_REFERENCIAS.NRO_REFERENCIA LIKE'" & txtBusqueda.Text & "%' ORDER BY AUD_REFERENCIAS.NRO_REFERENCIA ASC SELECT CATEGORIA,NRO_REFERENCIA,DESCRIPCION FROM #TEMP_REFERENCIAS WHERE FILA>" & (Application("paginaActualBusqueda") - 1) * 5 & " AND FILA<=" & Application("paginaActualBusqueda") * 5 & " ORDER BY LEN(NRO_REFERENCIA),NRO_REFERENCIA ASC"
        unatablatemporaldebusqueda.getDataSet(Application("ultimoQueryBusqueda"))
        unasreferencias.dataSet = unatablatemporaldebusqueda.dataSet
        unasreferencias.fillGridView(GridViewData)
    End Sub
    ...
    Private Sub GridViewData_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridViewData.RowCommand
        If e.CommandName = "Eliminar" Then
            ...
            'Me falta volver a calcular las páginas
        End If
    End Sub
